...
1. **为什么选择 LangGraph 而非 Celery？**
   - LangGraph 提供状态机工作流，支持断点续传和可视化
   - 与 LangChain 无缝集成，便于调用 LLM
   - 状态可持久化，便于错误恢复和审计

2. **为什么选择 DeepSeek 而非 GPT？**
   - 国产大模型，数据不出境，符合合规要求
   - 中文理解能力强，简历解析准确率高
   - API 价格低廉，大幅降低运营成本

3. **为什么选择 ChromaDB 而非 Milvus？**
   - 轻量级，无需额外部署，降低运维复杂度
   - 适合中小规模数据（万级简历），性能足够
   - 与 LangChain 集成良好，开发效率高
...
graph TB
    ...
    subgraph ParseExtractNode["解析提取节点"]
        direction TB
        B1[解析文档<br/>PDF使用PyMuPDF<br/>DOCX使用python-docx]
        B2[提取文本内容<br/>保留格式信息]
        B3[提取图片<br/>检测人脸]
        B4[LLM提取信息<br/>姓名/学历/技能等]
        B5[人脸检测<br/>OpenCV Haar]
        B1 --> B2 --> B3 --> B4 --> B5
    end
    
    subgraph FilterNode["筛选判断节点"]
        direction TB
        C1[获取筛选条件<br/>学历/技能/年限等]
        C2[构建筛选Prompt<br/>结构化条件描述]
        C3[LLM判断<br/>语义理解匹配]
        C4[生成筛选原因<br/>详细说明理由]
        C1 --> C2 --> C3 --> C4
    end
    ...
    subgraph StoreNode["数据存储节点"]
        direction TB
        D1[加密敏感信息<br/>AES加密手机号/邮箱]
        D2[保存MySQL<br/>人才信息表]
        D3[上传MinIO<br/>简历照片]
        D4[向量存ChromaDB<br/>支持语义检索]
        D1 --> D2 --> D3 --> D4
    end
    ...
    subgraph CacheNode["缓存节点"]
        direction TB
        E1[缓存Redis<br/>处理结果]
        E2[更新任务状态<br/>completed/failed]
        E3[WebSocket推送<br/>实时进度通知]
        E1 --> E2 --> E3
    end
...
系统采用 JWT（JSON Web Token）进行身份认证，支持三级权限控制：

```mermaid
graph LR
    A[登录请求] --> B{验证用户名密码}
    B -->|成功| C[生成JWT Token]
    B -->|失败| D[返回错误<br/>用户名或密码错误]
    C --> E[返回Token<br/>有效期24小时]
    E --> F[后续请求携带Token<br/>Authorization: Bearer xxx]
    F --> G{验证Token}
    G -->|有效| H[允许访问<br/>根据角色过滤权限]
    G -->|无效/过期| I[拒绝访问<br/>401 Unauthorized]
```
...
| 角色 | 权限说明 |
|------|----------|
| admin | 系统管理员，拥有所有权限，包括用户管理 |
| hr | HR 人员，可以管理简历、筛选条件、查看分析报告 |
| viewer | 只读用户，只能查看简历列表和详情 |

**安全措施**：
- 密码使用 bcrypt 哈希存储，不可逆
- Token 有效期 24 小时，支持自动刷新
- 敏感操作需要二次验证

### 2. 简历管理模块

简历管理是系统的核心功能，覆盖上传、处理、查询、管理的完整流程：

```mermaid
graph TB
    subgraph 上传阶段
        A[选择文件] --> B[格式校验<br/>PDF/DOCX/大小限制]
        B --> C[批量上传<br/>最多50个文件]
        C --> D[后台异步处理]
    end
...
**上传功能特性**：
- 支持 PDF、DOCX 格式，单个文件最大 10MB
- 批量上传最多 50 个文件，后台异步处理
- WebSocket 实时推送处理进度
- 自动去重，相同内容简历不重复上传
...
graph LR
    ...
    B --> B1[专科/本科/硕士/博士<br/>支持多选]
    C --> C1[技能列表+熟练程度<br/>精通/熟练/了解]
    D --> D1[最低年限/最高年限<br/>范围筛选]
    E --> E1[985/211/双一流<br/>院校分级]
    F --> F1[自然语言描述<br/>LLM理解判断]
```
...
智能分析模块基于 RAG 技术，实现自然语言问答：

```mermaid
graph TB
    A[用户提问] --> B{问题类型识别}
    B -->|人才查询| C[向量检索<br/>召回相关简历]
    B -->|统计分析| D[数据聚合<br/>计算统计指标]
    B -->|推荐排序| E[相似度计算<br/>排序推荐]
    
    C --> F[召回Top-K简历]
    D --> G[生成统计报告<br/>图表数据]
    E --> H[按匹配度排序]
    
    F --> I[LLM生成回答<br/>自然语言描述]
    G --> I
    H --> I
    
    I --> J[返回结果<br/>答案+来源+图表]
```
...
| 问题类型 | 示例问题 | 返回结果 |
|----------|----------|----------|
| 人才查询 | "有哪些 5 年经验的 Java 开发？" | 候选人列表 + 匹配度 |
| 统计分析 | "本科学历占比多少？" | 统计数据 + 图表 |
| 技能推荐 | "这个岗位还需要什么技能？" | 技能建议 + 依据 |
| 对比分析 | "张三和李四谁更合适？" | 对比表格 + 推荐结论 |
...
erDiagram
    ...
    TalentInfo {
        ...
        string screening_status "筛选状态：qualified/unqualified"
